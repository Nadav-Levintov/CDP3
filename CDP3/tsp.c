...
#define PRINT_ERROR(msg, A, B)    printf("%s: %s, %d\n", msg,A,B); \
                                fflush(stdout)
...
#define MPI_EXEC(rc) if (rc != MPI_SUCCESS) { \
                    PRINT_ERROR("MPI_Error! ", __func__, __LINE__); \
...
#define DYANAMIC_WORKER_FACTOR 10000
...
void next_prefix(int prefix[], int prefix_length, int citiesNum, int step_size, bool* visited);
...
void
branch_and_bound(int citiesNum, int *current_path, int current_index, int bound, int initial_cost, bool* visited,
	int **adj_matrix, int *final_res, int *final_path);
...
void sendMyBetterResult(int rank, int num_of_processes, int my_lowest_cost, void *send_buffer, int send_buffer_size, MPI_Datatype cost_type);
...
*/
void
branch_and_bound(int citiesNum, int *current_path, int current_index, int bound, int initial_cost, bool* visited, int **adj_matrix, int *final_res, int *final_path) {
	...
	for (int i = 0; i < citiesNum; i++) {
		if (visited[i] == false) {
			...
			if (lower_bound + current_cost < *final_res) {
				...
				branch_and_bound(citiesNum, current_path, current_index + 1, lower_bound,
					current_cost, visited, adj_matrix, final_res, final_path);
			}
...
int calc_length_of_prefix(int citiesNum, int num_of_processes) {
	...
	if (result >= citiesNum) {
		return citiesNum - 1;//we don't want to send a full path because than there is no reason to use BNB
	}
